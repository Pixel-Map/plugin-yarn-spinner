...
Yanfly.EML.Game_Interpreter_pluginCommand = Game_Interpreter.prototype.pluginCommand;
...
const YEPStandardPadding = 18;
Window_EventMiniLabel.prototype.windowHeight = function () {
  var height = this.fittingHeight(1);
  height = Math.max(height, 36 + YEPStandardPadding * 2);
  return height;
};
...
Window_EventMiniLabel.prototype.refresh = function () {
  ...
  const defaultTextPadding = 6;
  this.contents.clear();
  var txWidth = this.textWidthEx(this._text);
  txWidth += defaultTextPadding * 2;
  var width = txWidth;
  this.width = Math.max(width, Yanfly.Param.EMWMinWidth);
  this.width += YEPStandardPadding * 2;
  this.height = this.windowHeight();
  this.createContents();
  var wx = (this.contents.width - txWidth) / 2;
  var wy = 0;
  this.drawTextEx(this._text, wx + defaultTextPadding, wy);
